...
public class FileManager {
    ...
    public ArrayList<Score> getHighScores(){
        ...
        ArrayList<Score> temp2 = new ArrayList<>();    
        Score newTemp;
        ...
        for (int i = 0; i < temp.size(); i++){
            ...
            newTemp=new Score(name_,Integer.parseInt(points));
            temp2.add(newTemp);
            System.out.println(name_+  "   "+points );
        }
        for (int i = 0; i <temp2.size(); i++)
            for (int j = 0; j < temp2.size()-i-1; j++)
                if (temp2.get(j).getScore() < temp2.get(j+1).getScore())
                {
                    // swap temp and arr[i]
                    Score change = temp2.get(j);
                    temp2.set(j, temp2.get(j+1));
                    temp2.set(j+1, change);
                }
        return temp2;
    }
